"""
Test de integración completa del pipeline (Pasos 2-8) con datos mock.
"""
import polars as pl
...
from salud_ips.consumos.informes_consumo import ConsumoPipeline


def crear_datos_mock_pipeline():
    """Crea datos mock completos simulando la salida de paso_2_carga."""
    base = {
    ...
    # CONSUMOS DE FACTURACIÓN (con facturación join)
    n = 30
    ...
    anulados = pl.DataFrame({
        **{k: v[:2] for k, v in base.items()},
        ...
        "MedicoRealiza": [None, None],  # Nulos para detectar
        "NoDocumento": [""] * 2,
    })

    # LIMPIEZA CONSUMOS FACTURACIÓN (igual a consumos_fact pero con Médicos válidos)
...
def test_pipeline_completo():
    print("=" * 70)
    print("TEST INTEGRACION COMPLETA: PASOS 2-8")
    print("=" * 70)

    # Crear pipeline
    pipeline = ConsumoPipeline("01/01/2024", "31/01/2024")

    # Inyectar datos mock (saltando paso 1 y 2)
    ...
    try:
        ...
        # Paso 7: Conciliación (puede fallar sin archivos reales, omitimos o mock)
        print("\n" + "-" * 50)
        print("EJECUTANDO PASO 7: CONCILIACIÓN (omitida - requiere archivos reales)")
        print("-" * 50)
        # pipeline.conciliacion()
        ...
        print("\n" + "=" * 70)
        print("[OK] PIPELINE COMPLETO (PASOS 3-8) EJECUTADO EXITOSAMENTE")
        print("=" * 70)
